...
def search_model_documentation(query: str) -> str:
    ...
    try:
        ...
        # --- CRITICAL FIX: Unmask the raw error so the LLM prints it ---
        return (
            "STATUS: SYSTEM_ERROR\n\n"
            f"DATABASE ERROR DETAILS: {type(exc).__name__} - {str(exc)}\n\n"
            "CRITICAL INSTRUCTION: You MUST repeat the DATABASE ERROR DETAILS above word-for-word to the user right now, and then STOP reasoning."
        )
    ...
    if not docs:
        ...
        return (
            ...
            "Do not search again.\n"
            "Tell the user you could not find information related to their question."
        )
